...
- **Фигуры** — буквы (`boomerang`, `n`, `r`, `j`, `m`, `w`, `k`), арка, лист, кольцо, полумесяц, пилюли, круги и набор M3-форм: `squircle` (суперэллипс), `hexagon`, `square`, `triangle`, `pentagon`, `diamond`, `arrow`, `puffy` и `softburst` — все скруглённые под Material Design 3.
...
- **Почти без повторов** — композиции снимаются по структурному «отпечатку» (набор фигур, роли, позиции, размеры): `-L auto --count N` пересобирает совпадения, поэтому одинаковые обои практически исключены.
